...
const addUserSala= ({id,username,sala}) => {
    ...
    const existingUser= salas.find((user) => salas.sala === sala.sala && salas.users.username === username);
    if (existingUser){
        return { error: 'Username is already in that sala'};
    }
    
    const user = getUserLobby(username);
